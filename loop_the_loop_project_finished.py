...
def r_trajectory(velocity,height_of_track,angle):
	...
	Computes trajectory and plots that from given height
	if the input looks like r_trajectory(th_velocity(0.689),0.3,60).
	'''
...
def optimal_angle(velocity,height_of_track):    
    '''
    (int) -> int
    
    Return optimal angle in degree.
    '''  
    ...
    while guess - maximal_displacement > 0:
       print guess, 'guess'
       print maximal_displacement, 'maximal_displacement'
       guess = maximal_displacement
       angle += epsilon
       print angle, 'angle'
       x = []
       ...
       for time in numpy.arange(0,fall_time,0.0001):
          x.append((split_vx(velocity, angle)*time)*100)
	  maximal_displacement = x[-1]-x[0]
       print maximal_displacement, 'maximal_displacement for utan'
